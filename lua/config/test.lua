...
local FileName = {
  ...
  },
  {
    provider = function(self)
      ...
      local readonly = not vim.api.nvim_buf_get_option(0, "modifiable") or vim.api.nvim_buf_get_option(0, "readonly")
      if readonly then
        return "[] "
      end
...
local GitBranch = {
  ...
  },
  {
    ...
    condition = function(self)
      return type(git_branch_status_cache) == "table" and type(git_branch_status_cache["ahead"]) == "number"
    end,
    hl = function(self)
  ...
  },
  {
    ...
    condition = function(self)
      return type(git_branch_status_cache) == "table" and type(git_branch_status_cache["behind"]) == "number"
    end,
    hl = function(self)
...
local function setup_colors(colorname)
  ...
  local git_change =
      color_hl.get_color_with_fallback({ "GitSignsChange", "GitGutterChange", "diffChanged", "DiffChange" }, "fg", yellow)
  local git_delete =
      color_hl.get_color_with_fallback({ "GitSignsDelete", "GitGutterDelete", "diffRemoved", "DiffDelete" }, "fg", red)
  ...
  normal_bg3 =
      get_color_with_lualine(has_lualine, lualine_theme, has_airline, airline_theme, "normal", "c", "bg", {}, "bg")
  normal_fg3 =
      get_color_with_lualine(has_lualine, lualine_theme, has_airline, airline_theme, "normal", "c", "fg", {}, "fg")
...
local function update_git_branch()
  ...
  spawn.run({ "git", "-c", "color.status=never", "status", "-b", "--porcelain=v2" }, {
    ...
    if not failed_get_status and tbl.tbl_get(git_completed, "code") == 0 and tbl.list_not_empty(status_info) then
      local branch_status = {}
