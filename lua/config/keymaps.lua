...
-- 添加到你的 init.lua 或相关配置文件
local function add_markdown_header(level)
  local line = vim.api.nvim_get_current_line()
  local header_prefix = string.rep("#", level) .. " "

  -- 如果已经是标题，先移除原有的 #
  local clean_line = line:gsub("^#+%s*", "")
  local new_line = header_prefix .. clean_line
...
vim.keymap.set("n", "<leader>h1", function()
  add_markdown_header(1)
end, { desc = "Add H1 header" })
vim.keymap.set("n", "<leader>h2", function()
  add_markdown_header(2)
end, { desc = "Add H2 header" })
vim.keymap.set("n", "<leader>h3", function()
  add_markdown_header(3)
end, { desc = "Add H3 header" })
vim.keymap.set("n", "<leader>h4", function()
  add_markdown_header(4)
end, { desc = "Add H4 header" })
vim.keymap.set("n", "<leader>h5", function()
  add_markdown_header(5)
end, { desc = "Add H5 header" })
vim.keymap.set("n", "<leader>h6", function()
  add_markdown_header(6)
end, { desc = "Add H6 header" })
...
local function cycle_header()
  ...
  -- 循环到下一级别 (1->2->3->4->5->6->0->1...)
  local next_level = (level % 6) + 1
  if level == 6 then
    next_level = 0
  end
  ...
  if next_level == 0 then
    ...
    add_markdown_header(next_level)
  end
end
