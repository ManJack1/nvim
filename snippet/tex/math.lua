...
--
-- -- 定义一个简单的 \item 生成函数
-- local function generate_items(args, snip)
...
--
--   s({
--     trig = "(%d+)iden", -- 正则表达式触发器，匹配 'iden' 前面的数字
--     snippetType = "autosnippet",
--     regTrig = true,
--   }, {
--     -- 动态生成 bmatrix 环境内的单位矩阵
--     ls.function_node(function(_, snip)
--       local n = tonumber(snip.captures[1]) -- 提取数字并转换为数字类型
--       if not n then
--         return ""
--       end
--
--       local rows = {}
--       table.insert(rows, "\\begin{bmatrix}")
--       for i = 1, n do
--         local row = {}
--         for j = 1, n do
--           if i == j then
--             table.insert(row, "1") -- 对角线元素为 1
--           else
--             table.insert(row, "0") -- 非对角线元素为 0
--           end
--         end
--         if i < n then
--           -- 除最后一行外，每行末尾添加换行符 `\\`
...
--       return rows
--     end, {}),
--   }, { condition = tex.in_mathzone }), -- 仅在数学环境中触发
--
--   -- 只在 itemize 环境中触发
--   ls.snippet({
--     trig = "\n", -- 触发器为回车键
...
--       return string.match(line, "\\begin{.*matrix}") or string.match(line, "&")
--     end,
--   }),
-- }
